...
> 当前版本为 `0.3.0 Preview`。它适合自有测试机的应用开发、兼容性验证和设备调试，不应被视为生产级设备管理平台。
...
| 工具链诊断 | 检测随包 `adb`、`scrcpy`、`ffmpeg`、`aapt2`、`diec`、`ios`（go-ios）、`ssh` 与 `scp` | 安装后默认无需配置 `PATH`；仍可显式选择经审核的替代工具，界面会显示实际路径与来源 |
...
| 应用包分析 | 解析 APK/IPA 的包名、应用名、版本/构建、最低系统、目标 SDK、架构、MD5、权限/隐私声明和图标；APK 额外显示常见壳、加固与混淆特征 | APK 元数据优先使用 `aapt2`，加固识别使用随包 Detect It Easy 3.21；所有关键字段、权限与识别结果均可一键复制。零命中只表示未匹配当前特征库 |
...
- 投屏、截图与录屏：工作台左侧始终保留紧凑纵向手机视图，右侧集中画面操作；全局顶部可原地切换当前设备，完整信息按需打开“设备管理”。截图可直接复制到剪贴板或保存到电脑。Android 录屏开始后持续计时，不设 20 秒固定时限；切换设备或页面仍继续录制，直到用户回到工作台点击停止并保存，或退出应用时由会话清理收尾。需要键鼠操控时再弹出 scrcpy 交互窗口。
...
- 分析或安装包：`应用 → 本地包`，选择 APK/IPA 后即可查看元数据；应用名、包名、版本、Build、MD5、ABI 和路径可逐项复制，也可复制整份摘要。APK 的已知加固特征会独立展示，再对当前设备执行安装。
- 管理 Android 应用：`应用 → 设备应用与导出`，行内可直接复制包名、启动、停止或导出 APK；清数据和卸载集中在“更多”。
...
- 本机工具：发行包自带 ADB、scrcpy/Server、最小 FFmpeg、AAPT2、Detect It Easy、go-ios 与受限 Mobius SSH/SFTP 客户端。它们不会从网页上下文直接执行。
...
Windows、Linux 与 macOS 安装包都包含对应平台、架构的受控工具集，普通用户无需另装 Android SDK、scrcpy、FFmpeg、AAPT2、APK 加固识别工具或 libimobiledevice，也无需配置这些程序的 `PATH`：
...
- Android 内嵌屏幕只在 `127.0.0.1` 临时端口上向单个客户端提供画面，URL 带随机会话路径和 128 位令牌；切换设备或离开视图时清理当前投屏链路，但用户启动的录屏会保留到显式停止，退出应用时再统一停止并收尾。
