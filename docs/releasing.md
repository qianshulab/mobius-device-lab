...
| Detect It Easy | 3.21，horsicq/DIE-engine 官方目标归档和源码 | 随包仅使用控制台 `diec`、APK/DEX 规则库与所需目标运行库；DIE 本体为 MIT，动态链接的 Qt 模块按其 LGPL/GPL 选项交付许可、可替换说明和精确对应源码 |
...
6. 确认 Windows x64、Linux x64、macOS arm64 和 macOS x64 四个 runner 的工具准备/逐文件清单/架构/版本冒烟测试全部通过；每个目标都必须通过 `diec` 原生架构、版本、规则库、扫描冒烟、Qt 运行库和随包许可材料验证。确认第三方源码归档存在且在总校验和中，再对实际安装包执行恶意软件扫描、SBOM 生成和签名验签。
